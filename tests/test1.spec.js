...
test('Test scenario 1', async ({ page }) => {
    ...
    await iframe.locator('smart-frame').hover()

    // Check if the caption is correctly displayed (bottom section of the SmartFrame)
    const caption = iframe.locator('.caption-box')
    ...
    await iframe.locator('a >> text="EMBED"').click()

    // Check if the layer opens
    await expect(iframe.locator('.share__script')).toBeVisible()
})
